...
# Focaccia: A modular framework for composing Zero Knowledge Proof features

This crate provides a framework (Focaccia) that enables defining "components" that support Zero Knowledge
...
The name Focaccia is whimsically based on an acronym: Fear Of Commitment And Composable Components
...
Note that Focaccia is a framework for combining and orchestrating ZKPs over committed values using
...
The structure of the `composable_committed_components` crate is summarised as follows:
  ...
  - `tests/`
      ...
      - `device_binding_with_proof_system.rs` test demonstrating device binding with the BBS+ selective-disclosure flow in the DNC proof system directly (without using the Focaccia framework)
...
Note that the Focaccia framework does not prescribe or limit how commitments are produced, nor how proofs
...
As with components, the Focaccia framework does not prescribe or limit how equalities of
